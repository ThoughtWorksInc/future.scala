...
class continuationSpec extends AsyncFreeSpec with Matchers with Inside with ContinuationToScalaFuture {
  "test in paralle" in {
    ...
    val pc0: ParallelContinuation[Unit] = Parallel(Continuation { (handler: Unit => Trampoline[Unit]) =>
      inside(handler0) {
        case None =>
          Trampoline.delay {
    ...
    val pc1: ParallelContinuation[Unit] = Parallel(Continuation { (handler: Unit => Trampoline[Unit]) =>
      inside(handler1) {
        case None =>
          Trampoline.delay {
